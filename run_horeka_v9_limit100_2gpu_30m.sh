#!/bin/bash
# Submit with:
#   sbatch run_horeka_v9_limit100_2gpu_30m.sh
#
# This HoreKa job runs the 100-image v9 child-consistency YFCC4K validation on 2 A100 GPUs for up to
# 30 minutes and prints the wall-clock runtime at the end of the Slurm log.

#SBATCH --job-name=geo-v9-100-2gpu
...
#SBATCH --output=/hkfs/work/workspace/scratch/tj3409-SichengZuo/Multi-agent-MLLM-geolocation/geo_pipeline/results/horeka_v9_limit100_2gpu_%j.out
#SBATCH --error=/hkfs/work/workspace/scratch/tj3409-SichengZuo/Multi-agent-MLLM-geolocation/geo_pipeline/results/horeka_v9_limit100_2gpu_%j.err
...
python -u geo_pipeline/evaluate.py \
  ...
  --start 0 \
  --limit 100 \
  --out geo_pipeline/results/horeka_v9_limit100_child_consistency.json

python geo_pipeline/analyze_results.py \
  --pred geo_pipeline/results/horeka_v9_limit100_child_consistency.json
