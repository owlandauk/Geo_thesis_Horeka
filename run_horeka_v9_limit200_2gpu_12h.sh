#!/bin/bash
# Submit with:
#   sbatch run_horeka_v9_limit200_2gpu_12h.sh
#
# This HoreKa job runs the 200-image v9 child-consistency YFCC4K validation on 2 A100 GPUs for up to
# 12 hours and prints the wall-clock runtime at the end of the Slurm log.

#SBATCH --job-name=geo-v9-200-2gpu
...
python -u geo_pipeline/evaluate.py \
  ...
  --start 0 \
  --limit 200 \
  --out geo_pipeline/results/horeka_v9_limit200_child_consistency.json

python geo_pipeline/analyze_results.py \
  --pred geo_pipeline/results/horeka_v9_limit200_child_consistency.json
