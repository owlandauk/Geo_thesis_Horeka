...
Existing scripts in this `horeka/` directory:
...
These scripts can be submitted from inside `horeka/`; they `cd` to the
repository root before running Python, and Slurm logs are written to the
absolute `geo_pipeline/results` directory on HoreKa.
