#!/bin/bash
# Submit with:
#   sbatch run_horeka_soft_child_full_2gpu_6h.sh
#
# This HoreKa job runs the soft-child full YFCC4K evaluation on 2 A100 GPUs for up to
# 6 hours and prints the wall-clock runtime at the end of the Slurm log.

#SBATCH --job-name=geo-soft-full-2gpu
...
#SBATCH --output=/hkfs/work/workspace/scratch/tj3409-SichengZuo/Multi-agent-MLLM-geolocation/geo_pipeline/results/horeka_soft_child_full_2gpu_%j.out
#SBATCH --error=/hkfs/work/workspace/scratch/tj3409-SichengZuo/Multi-agent-MLLM-geolocation/geo_pipeline/results/horeka_soft_child_full_2gpu_%j.err
...
python -u geo_pipeline/evaluate.py \
  --batch_size 8 \
  --out geo_pipeline/results/horeka_soft_child_full_2gpu.json

python geo_pipeline/analyze_results.py \
  --pred geo_pipeline/results/horeka_soft_child_full_2gpu.json
